...
class Agent(Thing):
    """Un agente es una subclase de Thing con un atributo de instancia
    requerido (también conocido como ranura), .program, que debe contener una
    función que toma un argumento, el percepto, y devuelve una acción. (Lo que
    cuenta como percepción o acción dependerá del entorno específico en el que 
    exista el agente). Tenga en cuenta que "programa" es una ranura, no un
    método. Si fuera un método, entonces el programa podría "engañar" y 
    observar aspectos del agente. No se supone que haga eso: el programa solo
    puede mirar las percepciones. Un programa de agentes que necesita un modelo
    del mundo (y del propio agente) tendrá que construir y mantener su propio 
    modelo. Hay una ranura opcional, .performance, que es un número que 
    proporciona la medida de rendimiento del agente en su entorno."""
